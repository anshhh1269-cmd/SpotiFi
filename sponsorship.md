...
Spotifi is a portable Spotify-inspired music player that I designed and built from scratch as a personal engineering project. The device integrates an ESP32, TFT display, custom electronics, and a 3D-printed
enclosure. Since every internal component had to fit precisely, I designed the enclosure entirely in CAD and required accurate custom 3D printing. Moreover, it consists of five switches; those are for volume/back/forward,
the fifth switch is for three modes which are character display, song display and oscilloscope display.
